  ...
  const handleSubmit = async (e) => {
    ...
    if (isUrlValid(userInput)) {
      const options = {
        ...
        body: JSON.stringify({ original: userInput }), // ✅ rename `url` → `original`

      };
      const response = await fetch(
        `${import.meta.env.VITE_BASE_API_URL}/api/url/shorten`,
        options
      ).then((res) => res.json());
      console.log("BASE_API_URL 👉", import.meta.env.VITE_BASE_API_URL);

      console.log("Response from backend ➡️", response);
      if (response.id) {
